...
> edit/write tools on opencode) on real repo paths; (b) DeepAgents' task tool → opencode's Task
> tool, unavailable on Codex (see "Subagent discipline" notes below); (c) upstream's post-run OKF and
...
- **[adapted, opencode only — Codex has no subagent tool]** Verify the completed wiki using general-purpose, read-only Task-tool subagents in two roles — a question-finder subagent and one or more answer-verifier subagents. **On Codex, perform this same verification pass yourself inline: draft the same list of verification questions, then answer each directly against the wiki and repository evidence, repairing any question you cannot fully answer before moving on.**
...
Your job is to inspect the relevant evidence, then produce documentation in the target repository's openwiki/ directory that is excellent for both humans and future agents.
